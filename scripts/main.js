...
async function renderTrackingTab() {
    const b = document.getElementById('fullTrackingBody');
    const nd = document.getElementById('fullTrackingNoData');
    if (!b) return;
    try {
        const tracking = await supabaseGet('tracking', '*', 'created_at', 'desc');
        if (!tracking || tracking.length === 0) {
        ...
        b.innerHTML = tracking.slice(0, 100).map(t =>
            `<tr>
                <td style="font-size:.8rem;color:var(--fg-dim);">${formatTime(t.created_at)}</td>
                <td>${t.location_city || 'Freetown'}, ${t.location_country || 'Sierra Leone'}</td>
                <td style="font-family:monospace;font-size:.75rem;">${t.ip_address || 'Unknown'}</td>
        ...
        console.warn('Tracking tab render error:', e);
    }
}
...
async function refreshAdminData() {
    try {
        ...
        ['renderRegistrations', 'renderProducts', 'renderStaff', 'renderCodes', 'renderStickers',
         'renderInfluencers', 'refreshTrackingData', 'renderTrackingTab', 'refreshSocialFeeds',
         'renderAllStickersAndAds', 'renderProductAds', 'renderCashPayments']
            .forEach(fn => {
...
function viewProduct(id) {
    supabaseGet('products', '*').then(products => {
        const p = (products || []).find(x => x.id === id);
        if (!p) return;
        supabaseUpdate('products', id, { views: (p.views || 0) + 1 }).catch(() => {});
        showToast(p.title + ' — Le ' + (p.price_leone ? p.price_leone.toLocaleString() : '0'), 'info');
    });
}
...
function buyProduct(id) {
    supabaseGet('products', '*').then(products => {
        const p = (products || []).find(x => x.id === id);
        const name = p ? p.title : 'this product';
        const msg = encodeURIComponent('Hello Stars Meet Investors! I would like to purchase: ' + name + '. Kindly guide me through the process.');
        window.open('https://wa.me/23231286286?text=' + msg, '_blank');
        trackAction('buy_click', { product: name });
    });
}
...
// Click outside any admin-modal box closes it (same pattern as login modal)
...
async function renderRegistrations() {
    ...
    try {
        ...
        if (nd) nd.style.display = 'none';
        const badgeClass = { pending: 'badge-pending', completed: 'badge-completed', rejected: 'badge-rejected' };
        b.innerHTML = regs.map(r => `
            <tr>
                <td style="font-family:monospace;font-size:.78rem;">${r.unique_id || '—'}</td>
                <td>${r.business_name || ''}</td>
                <td>${r.rep_name || ''}</td>
                <td>${r.email || ''}</td>
                <td>${r.industry || ''}</td>
                <td><span class="badge ${badgeClass[r.status] || 'badge-pending'}">${r.status || 'pending'}</span></td>
                <td style="font-size:.8rem;color:var(--fg-dim);">${formatDate(r.date || r.created_at)}</td>
                <td>
                    <button class="icon-btn" title="Edit" onclick="editRegistration('${r.id}')"><i class="fas fa-pen"></i></button>
                    <button class="icon-btn danger" title="Delete" onclick="deleteRegistrationRow('${r.id}')"><i class="fas fa-trash"></i></button>
                </td>
            </tr>`).join('');
    } catch (e) {
        console.warn('Registrations render error:', e);
    }
}
...
function openRegModal() {
    ...
    document.getElementById('regBusiness').value = '';
    document.getElementById('regRep').value = '';
    document.getElementById('regEmail').value = '';
    document.getElementById('regPhone').value = '';
    document.getElementById('regIndustry').value = '';
    document.getElementById('regStatus').value = 'pending';
    openModal('regModal');
}

async function editRegistration(id) {
    const regs = await supabaseGet('registrations', '*');
    const r = (regs || []).find(x => x.id === id);
    if (!r) return;
    document.getElementById('regModalTitle').textContent = 'Edit Registration';
    document.getElementById('regId').value = r.id;
    document.getElementById('regBusiness').value = r.business_name || '';
    document.getElementById('regRep').value = r.rep_name || '';
    document.getElementById('regEmail').value = r.email || '';
    document.getElementById('regPhone').value = r.phone || '';
    document.getElementById('regIndustry').value = r.industry || '';
    document.getElementById('regStatus').value = r.status || 'pending';
    openModal('regModal');
}
...
async function saveRegistration(e) {
    ...
    const record = {
        ...
        rep_name: document.getElementById('regRep').value.trim(),
    ...
    try {
        if (id) {
            await supabaseUpdate('registrations', id, record);
            showToast('Registration updated.', 'success');
        } else {
            record.unique_id = generateUniqueID();
            record.date = new Date().toISOString();
            await supabaseInsert('registrations', record);
...
async function deleteRegistrationRow(id) {
    if (!confirm('Delete this registration?')) return;
    await supabaseDelete('registrations', id);
    renderRegistrations();
    refreshAdminData();
    ...
    try {
        ...
        b.innerHTML = products.map(p => `
            <tr>
                <td>${p.title || ''}</td>
                <td>${p.category || ''}</td>
                <td>${p.price_leone ? Number(p.price_leone).toLocaleString() : '0'}</td>
                <td>${p.views || 0}</td>
                <td><span class="badge ${p.status === 'active' ? 'badge-active' : 'badge-inactive'}">${p.status || 'active'}</span></td>
                <td>
                    <button class="icon-btn" title="Edit" onclick="editProduct('${p.id}')"><i class="fas fa-pen"></i></button>
                    <button class="icon-btn danger" title="Delete" onclick="deleteProductRow('${p.id}')"><i class="fas fa-trash"></i></button>
                </td>
            </tr>`).join('');
    } catch (e) {
        console.warn('Products admin render error:', e);
    }
}
...
async function editProduct(id) {
    ...
    document.getElementById('productPrice').value = p.price_leone || 0;
...
async function saveProduct(e) {
    ...
    const record = {
        ...
        price_leone: Number(document.getElementById('productPrice').value) || 0,
        image_url: document.getElementById('productImage').value.trim(),
        video_url: document.getElementById('productVideo').value.trim(),
        status: document.getElementById('productStatus').value
    };
    try {
        if (id) {
            await supabaseUpdate('products', id, record);
            showToast('Product updated.', 'success');
        } else {
            record.views = 0;
            await supabaseInsert('products', record);
...
async function renderStickers() {
    ...
    if (!b) return;
    try {
        const stickers = await supabaseGet('stickers', '*', 'created_at', 'desc');
        ...
        if (nd) nd.style.display = 'none';
        b.innerHTML = stickers.map(s => `
            <tr>
                <td>${s.title || ''}</td>
                <td style="max-width:260px;">${s.message || ''}</td>
                <td><span class="badge ${s.active !== false ? 'badge-active' : 'badge-inactive'}">${s.active !== false ? 'Active' : 'Inactive'}</span></td>
                <td>
                    <button class="icon-btn" title="Edit" onclick="editSticker('${s.id}')"><i class="fas fa-pen"></i></button>
                    <button class="icon-btn danger" title="Delete" onclick="deleteStickerRow('${s.id}')"><i class="fas fa-trash"></i></button>
                </td>
            </tr>`).join('');
    } catch (e) {
        console.warn('Stickers admin render error:', e);
    }
}

function openStickerModal() {
    document.getElementById('stickerModalTitle').textContent = 'Add Sticker';
    document.getElementById('stickerId').value = '';
    document.getElementById('stickerTitle').value = '';
    document.getElementById('stickerMessage').value = '';
    document.getElementById('stickerImage').value = '';
    document.getElementById('stickerActive').value = 'true';
...
async function editSticker(id) {
    ...
    if (!s) return;
    document.getElementById('stickerModalTitle').textContent = 'Edit Sticker';
    document.getElementById('stickerId').value = s.id;
    document.getElementById('stickerTitle').value = s.title || '';
    document.getElementById('stickerMessage').value = s.message || '';
    document.getElementById('stickerImage').value = s.image_url || '';
    document.getElementById('stickerActive').value = String(s.active !== false);
    openModal('stickerModal');
}

async function saveSticker(e) {
    e.preventDefault();
    const id = document.getElementById('stickerId').value;
    const record = {
        title: document.getElementById('stickerTitle').value.trim(),
        message: document.getElementById('stickerMessage').value.trim(),
        image_url: document.getElementById('stickerImage').value.trim(),
        active: document.getElementById('stickerActive').value === 'true'
    };
    try {
        if (id) {
            await supabaseUpdate('stickers', id, record);
            showToast('Sticker updated.', 'success');
        } else {
            await supabaseInsert('stickers', record);
...
async function deleteStickerRow(id) {
    if (!confirm('Delete this sticker?')) return;
    await supabaseDelete('stickers', id);
    renderStickers();
    renderAllStickersAndAds();
...
// 📡 AI SOCIAL FEED
// ============================================================
// NOTE: Pulling live posts from 30+ real social platforms needs a
// developer API key/app approval from each platform individually,
// which can't be set up from inside this code. "Fetch All" below
// simulates that feed (and stores it in Supabase) so the tab is
// fully wired up — swap fetchAllSocialFeeds() for real API calls
// per platform whenever you have those keys.
const SOCIAL_PLATFORMS = ['Twitter/X', 'Instagram', 'Facebook', 'LinkedIn', 'TikTok', 'YouTube'];
const SOCIAL_SAMPLES = [
    'Exciting news from Stars Meet Investors — new cohort applications now open!',
    'Congratulations to our latest cohort for closing a major funding round.',
    "Don't miss our upcoming pitch session — reserve your spot today.",
    'Behind the scenes at our latest investor networking breakfast.',
    'Q&A: what investors are really looking for in a pitch deck.'
];

async function fetchAllSocialFeeds() {
    showToast('Fetching social feeds...', 'info');
    try {
        const records = SOCIAL_PLATFORMS.map(platform => ({
            platform,
            content: SOCIAL_SAMPLES[Math.floor(Math.random() * SOCIAL_SAMPLES.length)],
            fetched_at: new Date().toISOString()
        }));
        for (const r of records) {
            await supabaseInsert('social_feeds', r);
        }
        showToast('Social feeds updated.', 'success');
        refreshSocialFeeds();
        refreshAdminData();
    } catch (err) {
...
async function refreshSocialFeeds() {
    const b = document.getElementById('socialFeedBody');
    const nd = document.getElementById('socialFeedNoData');
    const dashContainer = document.getElementById('socialFeedContainer');
    try {
        const feeds = await supabaseGet('social_feeds', '*', 'fetched_at', 'desc');
        if (b) {
            if (!feeds || feeds.length === 0) {
                ...
                if (nd) nd.style.display = 'none';
                b.innerHTML = feeds.slice(0, 50).map(f => `
                    <tr>
                        <td>${f.platform || ''}</td>
                        <td style="max-width:340px;">${f.content || ''}</td>
                        <td style="font-size:.8rem;color:var(--fg-dim);">${formatTime(f.fetched_at)}</td>
                        <td><button class="icon-btn danger" title="Delete" onclick="deleteSocialFeedRow('${f.id}')"><i class="fas fa-trash"></i></button></td>
                    </tr>`).join('');
            }
        }
        if (dashContainer) {
            if (!feeds || feeds.length === 0) {
                dashContainer.innerHTML = '<div class="text-center py-8" style="color:var(--fg-dim);">Click "Fetch All" to load social feeds from 30+ platforms.</div>';
            } else {
                dashContainer.innerHTML = feeds.slice(0, 5).map(f =>
                    `<div style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.05);"><strong>${f.platform}</strong>: ${f.content}</div>`
                ).join('');
            }
        }
    } catch (e) {
        console.warn('Social feed render error:', e);
    }
}

async function deleteSocialFeedRow(id) {
    await supabaseDelete('social_feeds', id);
    refreshSocialFeeds();
...
async function renderInfluencers() {
    ...
    if (!b) return;
    try {
        const influencers = await supabaseGet('influencers', '*', 'created_at', 'desc');
        ...
        b.innerHTML = influencers.map(i => `
            <tr>
                <td>${i.name || ''}</td>
                <td style="font-family:monospace;">${i.code || ''}</td>
                <td>
                    <button class="icon-btn" title="Copy link" onclick="copyInfluencerLink('${i.code}')"><i class="fas fa-copy"></i></button>
                    <span style="font-size:.75rem;color:var(--fg-dim);">?ref=${i.code}</span>
                </td>
                <td style="font-size:.8rem;color:var(--fg-dim);">${formatDate(i.created_at)}</td>
                <td><button class="icon-btn danger" title="Delete" onclick="deleteInfluencerRow('${i.id}')"><i class="fas fa-trash"></i></button></td>
            </tr>`).join('');
    } catch (e) {
        console.warn('Influencers render error:', e);
    }
}
...
async function saveInfluencer(e) {
    ...
    try {
        await supabaseInsert('influencers', { name, code: generateCode() });
...
async function deleteInfluencerRow(id) {
    if (!confirm('Delete this influencer?')) return;
    await supabaseDelete('influencers', id);
    renderInfluencers();
    refreshAdminData();
    showToast('Influencer deleted.', 'info');
}
...
async function renderCashPayments() {
    const b = document.getElementById('cashBody');
    const nd = document.getElementById('cashNoData');
    if (!b) return;
    try {
        const payments = await supabaseGet('cash_payments', '*', 'date', 'desc');
        ...
        if (nd) nd.style.display = 'none';
        b.innerHTML = payments.map(c => `
            <tr>
                <td>${c.business_name || ''}</td>
                <td>${c.amount_leone ? Number(c.amount_leone).toLocaleString() : '0'}</td>
                <td>${c.received_by || ''}</td>
                <td style="font-size:.8rem;color:var(--fg-dim);">${formatDate(c.date || c.created_at)}</td>
                <td>${c.note || ''}</td>
                <td><button class="icon-btn danger" title="Delete" onclick="deleteCashRow('${c.id}')"><i class="fas fa-trash"></i></button></td>
            </tr>`).join('');
    } catch (e) {
        console.warn('Cash payments render error:', e);
    }
}

function openCashModal() {
    document.getElementById('cashBusiness').value = '';
    document.getElementById('cashAmount').value = '';
    document.getElementById('cashReceivedBy').value = currentUser ? (currentUser.name || currentUser.username) : '';
    document.getElementById('cashNote').value = '';
    openModal('cashModal');
}

async function saveCashPayment(e) {
    e.preventDefault();
    const record = {
        business_name: document.getElementById('cashBusiness').value.trim(),
        amount_leone: Number(document.getElementById('cashAmount').value) || 0,
        received_by: document.getElementById('cashReceivedBy').value.trim(),
        note: document.getElementById('cashNote').value.trim(),
        date: new Date().toISOString()
    };
    try {
        await supabaseInsert('cash_payments', record);
...
async function deleteCashRow(id) {
    if (!confirm('Delete this payment record?')) return;
    await supabaseDelete('cash_payments', id);
    renderCashPayments();
    ...
    try {
        ...
        b.innerHTML = users.map(u => `
            <tr>
                <td>${u.name || ''}</td>
                <td>${u.username || ''}</td>
                <td><span class="badge badge-active">${u.role || 'staff'}</span></td>
                <td style="font-size:.8rem;color:var(--fg-dim);">${formatDate(u.created_at)}</td>
                <td>
                    <button class="icon-btn" title="Edit" onclick="editStaff('${u.id}')"><i class="fas fa-pen"></i></button>
                    <button class="icon-btn danger" title="Delete" onclick="deleteStaffRow('${u.id}')"><i class="fas fa-trash"></i></button>
                </td>
            </tr>`).join('');
    } catch (e) {
...
async function saveStaff(e) {
    ...
    try {
        if (id) {
            await supabaseUpdate('users', id, record);
            showToast('Staff account updated.', 'success');
        } else {
            await supabaseInsert('users', record);
            showToast('Staff account added.', 'success');
        }
...
async function renderCodes() {
    const b = document.getElementById('codesBody');
    const nd = document.getElementById('codesNoData');
    if (!b) return;
    try {
        const codes = await supabaseGet('codes', '*', 'created_at', 'desc');
        ...
        b.innerHTML = codes.map(c => `
            <tr>
                ...
                <td>
                    <button class="icon-btn" title="Toggle active" onclick="toggleCodeActive('${c.id}', ${c.active === false})"><i class="fas fa-toggle-on"></i></button>
                    <button class="icon-btn danger" title="Delete" onclick="deleteCodeRow('${c.id}')"><i class="fas fa-trash"></i></button>
                </td>
            </tr>`).join('');
    } catch (e) {
        console.warn('Codes render error:', e);
    }
}

function openCodeModal() {
    document.getElementById('codeType').value = 'referral';
...
async function saveCode(e) {
    ...
    const record = {
        ...
        value: document.getElementById('codeValue').value.trim(),
        active: true,
        uses: 0
    };
    try {
        await supabaseInsert('codes', record);
        showToast('Code generated: ' + record.code, 'success');
...
async function toggleCodeActive(id, makeActive) {
    await supabaseUpdate('codes', id, { active: makeActive });
    renderCodes();
}

async function deleteCodeRow(id) {
    if (!confirm('Delete this code?')) return;
    await supabaseDelete('codes', id);
    renderCodes();
